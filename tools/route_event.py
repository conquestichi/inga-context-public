...
# 最小パーサ: EVENT_MAP.yaml の kpi_guardrail_breach 節に "notify: slack" / "risk: default" があれば実行
def read_event_map(path):
  route = {"notify": None, "risk": None}
  key = None
  for raw in open(path, encoding="utf-8"):
    s = raw.rstrip("\n")
    if s.strip().startswith("#") or not s.strip(): continue
    if s.strip() == "routes:": continue
    if "kpi_guardrail_breach:" in s:
      key = "kpi_guardrail_breach"; continue
...
def main():
  ...
  typ = ev.get("event","")
  if typ != "kpi_guardrail_breach":
    print(f"[route] skip unknown event: {typ}"); return
  emap = read_event_map("/root/inga-context-public/context/EVENT_MAP.yaml")
  if emap.get("notify") == "slack":
